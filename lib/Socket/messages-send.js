...
import { areJidsSameUser, getBinaryNodeChild, getBinaryNodeChildren, getBizBinaryNode, isHostedLidUser, isHostedPnUser, isJidBot, isJidGroup, isJidMetaAI, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, PSA_WID, S_WHATSAPP_NET } from '../WABinary/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            // Biz Binary Node: required for buttons/list/template/native-flow fanout and the secureMetaServiceLabel biz shelf
            if ((!alreadyHasBizNode && shouldIncludeBizBinaryNode(normalizedMsg)) || addBizAttributes) {
                stanza.content.push(getBizBinaryNode(normalizedMsg));
            }
            logger.debug({ msgId }, `sending message to ${participants.length} devices`);
            await sendNode(stanza);
            // Fire-and-forget: issue our token to the contact AFTER message send.
            // WA Web skips protocol messages and PSA/bot contacts (TcTokenChatAction: isRegularUser)
            const isProtocolMsg = !!normalizedMsg?.protocolMessage;
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                if (isDeleteMsg) {
                    ...
                    // AI message label — WhatsApp only supports it in private chats
