...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, bytesToCrockford, configureSuccessfulPairing, Curve, derivePairingCodeKey, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, handleCompanionRegRefresh, normalizePairingPhoneNumber, makeEventBuffer, makePairingQRRenderer, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
    ...
    /** true once the noise handshake finished and the socket can carry IQs */
    let handshakeComplete = false;
    const handshakeWaiters = new Set();
    /** in-flight requestPairingCode() promise (single-flight) */
    let pairingCodeRequest;
    /** creds.me was minted by requestPairingCode() and is not confirmed yet */
    let pairingProvisional = false;
    /** pair-success / login seen — the provisional identity is now real */
    let pairingCompleted = false;
    /**
     * Revert the half-written identity created by requestPairingCode().
     *
     * A pairing attempt that fails (515, 401, timeout, wrong number…) used to
     * leave `creds.me` pointing at a JID the server never confirmed. Every
     * later connect then tried to *log in* with that JID instead of
     * registering, and failed again — the "cannot pairing until I delete the
     * session folder" loop.
     */
    const rollbackProvisionalPairing = () => {
        if (!pairingProvisional || pairingCompleted || creds.registered) {
            return;
        }
        pairingProvisional = false;
        creds.me = undefined;
        creds.pairingCode = undefined;
        logger.warn('rolled back an unconfirmed pairing session; the next connect will register cleanly');
        ev.emit('creds.update', { me: undefined, pairingCode: undefined });
    };
    /** resolve once the noise handshake is done (or fail after connectTimeoutMs) */
    const waitForHandshake = (timeoutMs = connectTimeoutMs) => {
        ...
        return promiseTimeout(timeoutMs, (resolve, reject) => {
            const waiter = (err) => (err ? reject(err) : resolve());
            handshakeWaiters.add(waiter);
            // stop waiting if the socket dies underneath us
            ws.once('close', () => {
                if (handshakeWaiters.delete(waiter)) {
                    reject(new Boom('Connection Closed', { statusCode: DisconnectReason.connectionClosed }));
                }
            });
        });
    };
    const settleHandshakeWaiters = (err) => {
        handshakeComplete = handshakeComplete || !err;
    ...
    // never leave a failed/aborted pairing-code attempt half-written in creds
    socketEndHandlers.push(() => {
        settleHandshakeWaiters(new Boom('Connection Closed', { statusCode: DisconnectReason.connectionClosed }));
        rollbackProvisionalPairing();
    });
    ...
    const getAvailablePreKeysOnServer = async () => {
        ...
        if (!countChild) {
            // no usable answer: report 0 so the caller uploads a fresh batch
            // instead of crashing on `undefined.attrs`.
            logger?.warn?.('encrypt/count returned no count node; assuming 0 pre-keys on server');
            return 0;
        }
        const value = Number(countChild.attrs?.value);
    ...
    const uploadPreKeys = async (count = MIN_PREKEY_COUNT) => {
        ...
        const uploadLogic = async (retryCount) => {
            // Circuit breaker: retrying against a socket that is already gone
            // just burns exponential backoff and leaves the session degraded
            // until it needs a full re-pair (#2784).
            if (closed || !ws.isOpen) {
                throw new Boom('Connection Closed', { statusCode: DisconnectReason.connectionClosed });
            }
            ...
            try {
                ...
                // Give up immediately when the socket is gone instead of backing
                // off against a dead connection.
                if (closed || !ws.isOpen) {
                    logger.warn({ count, retryCount }, 'aborting pre-key upload: connection no longer open');
                    throw uploadError;
                }
                if (retryCount < 3) {
        ...
        const uploadTimeoutPromise = new Promise((_, reject) => {
            uploadTimeout = setTimeout(() => reject(new Boom('Pre-key upload timeout', { statusCode: DisconnectReason.timedOut })), UPLOAD_TIMEOUT);
        });
        uploadPreKeysPromise = Promise.race([uploadLogic(0), uploadTimeoutPromise]);
        try {
            await uploadPreKeysPromise;
        }
        finally {
    ...
    /**
     * Link this device with an 8-character pairing code instead of a QR.
     *
     * Hardened against the failure modes reported in
     * WhiskeySockets/Baileys#2512 (`stream:error 515` / "Couldn't link device"):
     *  - the phone number is normalised first ("+62 812-…" is accepted)
     *  - calls are single-flight: a bot that asks for a code on every
     *    `connection.update` no longer fires a second, conflicting request
     *  - it waits for the noise handshake instead of throwing
     *    "Connection Closed" when called too early
     *  - a failed attempt rolls the provisional creds back, so the next
     *    connect registers cleanly instead of reusing a rejected identity
     *
     * @param {string | number} phoneNumber digits, optionally with '+', spaces or dashes
     * @param {string} [customPairingCode] exactly 8 chars
     * @param {object} [options]
     * @param {string} [options.defaultCountryCode] expands a leading national zero
     */
    const requestPairingCode = async (phoneNumber, customPairingCode, options = {}) => {
        // single-flight: repeated calls return the code already pending
        if (pairingCodeRequest) {
            logger.debug('pairing code already requested on this socket, reusing the pending request');
            return pairingCodeRequest;
        }
        const attempt = (async () => {
            try {
                if (creds.registered) {
                    throw new Boom('This session is already registered — logout or use a fresh auth state to pair again', { statusCode: DisconnectReason.loggedOut });
                }
                const normalized = normalizePairingPhoneNumber(phoneNumber, options);
                if (!normalized) {
                    throw new Boom(`Invalid phone number for pairing: ${String(phoneNumber)}`, { statusCode: 400 });
                }
                if (customPairingCode !== undefined && customPairingCode !== null) {
                    if (typeof customPairingCode !== 'string' || customPairingCode.length !== 8) {
                        throw new Boom('Custom pairing code must be exactly 8 chars', { statusCode: 400 });
                    }
                }
                // never send before the handshake is up: sending earlier throws
                // 'Connection Closed', which used to surface as a pairing failure
                await waitForHandshake();
                const pairingCode = customPairingCode ?? bytesToCrockford(randomBytes(5));
                authState.creds.pairingCode = pairingCode;
                authState.creds.me = {
                    id: jidEncode(normalized, 's.whatsapp.net'),
                    name: '~'
                };
                pairingProvisional = true;
        ...
        pairingCodeRequest = attempt;
        try {
            return await attempt;
        }
        catch (error) {
            // Free the slot so a corrected number — or a fresh attempt after a
            // reconnect — can retry. Clearing it inside the async body above
            // would not work: that body can run to completion synchronously
            // (e.g. an invalid number throws before the first await), so the
            // assignment on the next line would just re-pin the stale promise
            // and every later call would replay this first error forever.
            if (pairingCodeRequest === attempt) {
                pairingCodeRequest = undefined;
            }
            throw error;
        }
    };
