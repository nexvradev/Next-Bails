...
export const makeChatsSocket = (config) => {
    ...
    const fetchPrivacySettings = async (force = false) => {
        if (!privacySettings || force) {
            // query() resolves undefined when the IQ never gets a response
            // (timeout, or the socket being torn down mid-query). Destructuring
            // that straight away threw an uncatchable TypeError out of
            // executeInitQueries — see WhiskeySockets/Baileys#2784.
            ...
            if (!result) {
                logger?.warn?.('privacy query returned no result; keeping previous privacy settings');
    ...
    const executeInitQueries = async () => {
        // Individually guarded: a single failing init query (a torn-down socket,
        // a timed-out IQ) used to reject Promise.all, and because this is fired
        // from a `connection.update` listener the rejection escaped as an
        // unhandled exception and took the whole process down (#2784).
        const initQueries = [
            ['props', fetchProps],
            ['blocklist', fetchBlocklist],
            ['privacy', fetchPrivacySettings]
        ];
        await Promise.all(initQueries.map(async ([name, run]) => {
            try {
                await run();
            }
            catch (error) {
                logger?.warn?.({ error, query: name }, 'init query failed; continuing without it');
            }
        }));
    };
