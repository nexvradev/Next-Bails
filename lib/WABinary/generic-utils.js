...
/**
 * Native-flow names WhatsApp recognises as first-party flows.
 * Derived empirically; unknown names fall through to the generic "mixed" node.
 */
const FLOWS_MAP = {
    mpm: true,
    cta_catalog: true,
...
/**
 * Build the `<biz>` binary node for interactive/business-labelled messages.
 * NOTE: the "v" and "name" values influence how WhatsApp renders & validates flows.
 * @param {object} message normalized message content (post-normalizeMessageContent)
 * @returns {object} binary node with shape { tag, attrs, [content] }
 */
