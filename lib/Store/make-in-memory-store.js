...
export const makeInMemoryStore = (config = {}) => {
    ...
    const KeyedDBCtor = KeyedDB?.default ?? KeyedDB; // handle CJS <-> ESM default export differences
    const chats = new KeyedDBCtor(chatKey, c => c.id);
    const messages = {};
    const contacts = {};
    const groupMetadata = {};
    const presences = {};
    ...
    const labelAssociations = new KeyedDBCtor(labelAssociationKey, labelAssociationKey.key);
