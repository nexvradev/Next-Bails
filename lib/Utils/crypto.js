...
export function hkdf(buffer, expandedLength, info = {}) {
    const salt = info.salt ? (Buffer.isBuffer(info.salt) ? info.salt : Buffer.from(info.salt)) : Buffer.alloc(0);
    const infoBuf = info.info ? (Buffer.isBuffer(info.info) ? info.info : Buffer.from(info.info)) : Buffer.alloc(0);
    const ikm = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer);
    return Buffer.from(hkdfSync('sha256', ikm, salt, infoBuf, expandedLength));
}
